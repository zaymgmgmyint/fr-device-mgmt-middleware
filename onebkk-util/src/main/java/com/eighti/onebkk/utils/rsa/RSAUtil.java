package com.eighti.onebkk.utils.rsa;

import java.security.KeyFactory;
import java.security.PrivateKey;
import java.security.PublicKey;
import java.security.spec.PKCS8EncodedKeySpec;
import java.security.spec.X509EncodedKeySpec;
import java.util.Base64;

import javax.crypto.Cipher;

public class RSAUtil {

	// Sanitize the PEM key by replacing \\n with \n
	public static String sanitizeKey(String key) {
		return key.replace("\\n", "\n");
	}

	// Convert PEM-encoded public key to PublicKey object
	public static PublicKey importPublicKey(String pem) throws Exception {
		String publicKeyPEM = pem.replace("-----BEGIN PUBLIC KEY-----", "").replace("-----END PUBLIC KEY-----", "")
				.replaceAll("\\s+", ""); // remove whitespace/newlines

		byte[] encoded = Base64.getDecoder().decode(publicKeyPEM);
		X509EncodedKeySpec keySpec = new X509EncodedKeySpec(encoded);
		KeyFactory keyFactory = KeyFactory.getInstance("RSA");
		return keyFactory.generatePublic(keySpec);
	}

	// Convert PEM-encoded private key to PrivateKey object
	public static PrivateKey importPrivateKey(String pem) throws Exception {
		String privateKeyPEM = pem.replace("-----BEGIN PRIVATE KEY-----", "").replace("-----END PRIVATE KEY-----", "")
				.replaceAll("\\s+", ""); // remove whitespace/newlines

		byte[] encoded = Base64.getDecoder().decode(privateKeyPEM);
		PKCS8EncodedKeySpec keySpec = new PKCS8EncodedKeySpec(encoded);
		KeyFactory keyFactory = KeyFactory.getInstance("RSA");
		return keyFactory.generatePrivate(keySpec);
	}

	// Decrypt data with the RSA private key
	public static String decryptWithPrivateKey(PrivateKey privateKey, byte[] encryptedData) throws Exception {
		Cipher cipher = Cipher.getInstance("RSA/ECB/OAEPWithSHA-256AndMGF1Padding");
		cipher.init(Cipher.DECRYPT_MODE, privateKey);
		byte[] decryptedData = cipher.doFinal(encryptedData);
		return new String(decryptedData); // assuming the decrypted data is a UTF-8 encoded JSON string
	}

	public static void main(String[] args) {

		try {

			// Sample private key PEM (sanitized)
			String privateKeyPem = "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----";

			// Sanitize and import the private key
			privateKeyPem = RSAUtil.sanitizeKey(privateKeyPem);
			PrivateKey privateKey = RSAUtil.importPrivateKey(privateKeyPem);

			// Encrypted data as a Base64 string
			String encryptedDataBase64 = "";

			// Convert Base64 string to byte array
			byte[] encryptedData = Base64.getDecoder().decode(encryptedDataBase64);

			// Decrypt data
			String decryptedText = RSAUtil.decryptWithPrivateKey(privateKey, encryptedData);
			System.out.println("Decrypted Text: " + decryptedText);

		} catch (Exception e) {
			e.printStackTrace();
		}
	}
}
